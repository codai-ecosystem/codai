		...
		>
			...
			<div className="p-6 border-b border-gray-200 dark:border-gray-700">
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
					...
					<button
						onClick={() =>
							storeMemory('Test memory from API Gateway', { source: 'codai-platform' })
						}
						...
						requests.map(request => (
							...
							>
								<div className="flex items-center space-x-3">
									...
									<div>
										<div className="flex items-center space-x-2">
											...
											<span
												className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${getMethodColor(request.method)}`}
											>
												{request.method}
											</span>
										</div>
								...
								<div className="flex items-center space-x-3 text-xs text-gray-500 dark:text-gray-400">
									...
									{request.statusCode && (
										<span
											className={`inline-flex items-center px-2 py-0.5 rounded border ${getStatusColor(request.status)}`}
										>
											{request.statusCode}
										</span>
									)}
