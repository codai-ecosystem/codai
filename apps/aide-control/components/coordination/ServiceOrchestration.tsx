...
export function ServiceOrchestration({ className = '' }: ServiceOrchestrationProps) {
	...
	const servicesByPriority = services.reduce(
		(acc, service) => {
			...
			<div className="p-6 space-y-6">
				{Object.entries(priorityLabels).map(([priority, label]) => {
					...
					return (
						<div
							key={priority}
							className={`rounded-lg border p-4 ${getPriorityColor(Number(priority))}`}
						>
								...
								{priorityServices.map(service => (
									...
									>
										<div className="flex items-center justify-between mb-2">
											...
											<span
												className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border ${getStatusColor(service.status)}`}
											>
												{service.status}
											</span>
										</div>
