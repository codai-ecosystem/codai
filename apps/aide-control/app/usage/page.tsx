...
export default function UsagePage() {
	...
	const totalInputTokens = usageRecords.reduce(
		(sum, record) => sum + (record.requestDetails.inputTokens || 0),
		0
	);
	const totalOutputTokens = usageRecords.reduce(
		(sum, record) => sum + (record.requestDetails.outputTokens || 0),
		0
	);
							...
							{usageRecords.map((record, index) => (
								<tr key={index} className="hover:bg-gray-50 dark:hover:bg-gray-700">
									...
									<td className="border px-4 py-2">
										...
										{record.requestDetails.outputTokens
											? ` / ${record.requestDetails.outputTokens.toLocaleString()} out`
											: ''}
									</td>
