...
describe('LocalStorageAdapter', () => {
	...
	it('should throw an error for unsupported import formats', async () => {
		await expect(adapter.importGraph('<xml></xml>', 'xml')).rejects.toThrow(
...
describe('PersistenceAdapter factory', () => {
	...
	it('should throw an error for invalid adapter types', () => {
		expect(() => createPersistenceAdapter('invalid' as any)).toThrow(
			'Unsupported persistence adapter type'
		);
	});
});
