  ...
  export function TaskGraph({ taskId }: { taskId: string }) {
  	...
  	return (
  		<div className="task-graph-container">
  			...
  			<div className="task-legend">{/* Legend items */}</div>
  		</div>
  	);
  }
  ```
  ...
  export default function Dashboard() {
  	...
  	useEffect(() => {
  		...
  		eventSource.addEventListener('agent-update', event => {
  			...
  			setActiveAgents(prev => prev.map(a => (a.id === updatedAgent.id ? updatedAgent : a)));
  		});
  		eventSource.addEventListener('task-update', event => {
  			...
  			setRecentTasks(prev => {
  				...
  				if (existing >= 0) {
  					return [...prev.slice(0, existing), updatedTask, ...prev.slice(existing + 1)];
  				}
  ...
  export function withAuth(handler: AuthenticatedHandler) {
  	return async (req: NextRequest) => {
  		...
  		try {
  			...
  			if (handler.requiredRole && !hasRequiredRole(user, handler.requiredRole)) {
...
describe('Agent API Integration Tests', () => {
	...
	describe('POST /api/agents', () => {
		it('should create and start a new task', async () => {
			...
			const taskDoc = await firebase.firestore().collection('agent_tasks').doc(taskId).get();

			expect(taskDoc.exists).toBe(true);
			expect(taskDoc.data()).toEqual(
				expect.objectContaining({
  ...
  export class OptimizedQueries {
  	static async getUserDashboard(userId: string): Promise<DashboardData> {
  		...
  		const tasksRef = db
  			...
  			.limit(10);
  		const projectsRef = db.collection('projects').where('userId', '==', userId).limit(5);
  		const usageRef = db
  			.collection('usage')
  			.where('userId', '==', userId)
