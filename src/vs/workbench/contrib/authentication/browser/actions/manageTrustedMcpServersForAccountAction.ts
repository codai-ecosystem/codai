...
class ManageTrustedMcpServersForAccountActionImpl {
	...
	private async _getItems(providerId: string, accountLabel: string) {
		let allowedMcpServers = this._mcpServerAuthenticationAccessService.readAllowedMcpServers(
			providerId,
			accountLabel
		);
		...
		const resolvedMcpServers = await Promise.all(
			allowedMcpServers.map(server =>
				this._mcpServerService.servers.get().find(s => s.definition.id === server.id)
			)
		);
		...
		if (!allowedMcpServers.length) {
			this._dialogService.info(
				localize('noTrustedMcpServers', 'This account has not been used by any MCP servers.')
			);
			return [];
		}
