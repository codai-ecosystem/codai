...
export class LanguageModelToolsService extends Disposable implements ILanguageModelToolsService {
			...
			if (dto.context) {
				...
				if (
					...
					const result = await this._dialogService.confirm({
						message: renderStringAsPlaintext(prepared.confirmationMessages.title),
	...
	private playAccessibilitySignal(toolInvocations: ChatToolInvocation[]): void {
		...
		const setting: { sound: 'auto' | 'on' | 'off' } = this._configurationService.getValue(
			AccessibilitySignal.chatUserActionRequired.settingsKey
		);
		if (
			setting.sound === 'on' ||
			(setting.sound === 'auto' &&
				(this._accessibilityService.isScreenReaderOptimized() || !hasFocusedWindow))
		) {
			this._accessibilitySignalService.playSignal(AccessibilitySignal.chatUserActionRequired, {
	...
	toEnablementMap(toolOrToolsetNames: Iterable<string>): Record<string, boolean> {
		...
		for (const tool of this._tools.values()) {
			if (
				(tool.data.toolReferenceName && toolOrToolset.has(tool.data.toolReferenceName)) ||
	...
	createToolSet(
		...
		const result = new (class extends ToolSet implements IDisposable {
			...
			source,
			options?.toolReferenceName ?? displayName,
			options?.description
		);
