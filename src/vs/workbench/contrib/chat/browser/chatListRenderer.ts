	...
	private renderChatErrorDetails(
		...
		if (content.errorDetails.isQuotaExceeded) {
			...
			errorConfirmation.addDisposable(
				errorConfirmation.onDidChangeHeight(() => this.updateItemHeight(templateData))
			);
			return errorConfirmation;
		} else {
			const level =
				content.errorDetails.level ??
				(content.errorDetails.responseIsFiltered ? ChatErrorLevel.Info : ChatErrorLevel.Error);
			return this.instantiationService.createInstance(
				ChatErrorContentPart,
				level,
				new MarkdownString(content.errorDetails.message),
