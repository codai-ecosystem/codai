...
class SetupAgent extends Disposable implements IChatAgentImplementation {
	...
	static registerVSCodeAgent(
		...
		return instantiationService.invokeFunction(accessor => {
			...
			const { agent, disposable } = SetupAgent.doRegisterAgent(
				...
				'vscode',
				false,
				localize2('vscodeAgentDescription', 'Ask questions about VS Code').value,
	...
	private static doRegisterAgent(
		...
		const agent = disposables.add(
			instantiationService.createInstance(SetupAgent, context, controller, location)
		);
	...
	private async doInvokeWithSetup(
		...
		try {
			result = await ChatSetup.getInstance(
				...
				this.context,
				this.controller
			).run({ disableChatViewReveal: true /* we are already in a chat context */ });
		} catch (error) {
			...
			if (result.success) {
				if (result.dialogSkipped) {
					progress({
						...
						content: new MarkdownString(
							localize('copilotSetupSuccess', 'Copilot setup finished successfully.')
						),
					});
...
class ChatSetup {
			...
			switch (setupStrategy) {
				...
				case ChatSetupStrategy.Canceled:
					this.telemetryService.publicLog2<InstallChatEvent, InstallChatClassification>(
						...
						{ installResult: 'failedMaybeLater', installDuration: 0, signUpErrorCode: undefined }
					);
					break;
			}
		} catch (error) {
				...
				createWorkbenchDialogOptions(
					{
						...
						renderFooter:
							this.telemetryService.telemetryLevel !== TelemetryLevel.NONE
								? footer => footer.appendChild(this.createDialogFooter(disposables))
								: undefined,
	...
	private getButtons(
	...
	): Array<[string, ChatSetupStrategy, { styleButton?: (button: IButton) => void } | undefined]> {
		let buttons: Array<
			[string, ChatSetupStrategy, { styleButton?: (button: IButton) => void } | undefined]
		>;

		if (this.context.state.entitlement === ChatEntitlement.Unknown) {
			const supportAlternateProvider =
				this.configurationService.getValue('chat.setup.signInWithAlternateProvider') === true &&
							...
							ChatSetupStrategy.SetupWithAccountCreate,
							{
								styleButton: button => {
									...
									separator?.appendChild(
										$('.buttons-separator-center', undefined, localize('or', 'Or'))
									);
						...
						buttons = coalesce([
							...
							[
								variant !== 'account-create'
									...
									: localize('continueWithProvider', 'Continue with {0}', defaultChat.providerName),
								ChatSetupStrategy.SetupWithoutEnterpriseProvider,
								{
									styleButton: button => {
		...
		buttons.push([
			...
			{ styleButton: button => button.element.classList.add('link-button', 'skip-button') },
		]);
	...
	private getDialogTitle(
		variant: 'default' | 'brand-gh' | 'brand-vsc' | 'style-glow' | 'alt-first' | unknown
	): string {
		if (this.context.state.entitlement === ChatEntitlement.Unknown) {
			switch (variant) {
				case 'brand-gh':
	...
	private createDialogFooter(disposables: DisposableStore): HTMLElement {
		...
		element.appendChild(
			$(
				'p',
				undefined,
				disposables.add(markdown.render(new MarkdownString(settings, { isTrusted: true }))).element
			)
		);
		...
		const updateRegistration = () => {
			if (!context.state.hidden && !context.state.disabled) {
				...
				if (
					...
					disposables.add(
						SetupAgent.registerVSCodeAgent(this.instantiationService, context, controller)
							.disposable
					);
				}
			} else {
		...
		class ChatSetupFromAccountsAction extends Action2 {
			...
			override async run(accessor: ServicesAccessor): Promise<void> {
				...
				telemetryService.publicLog2<
					WorkbenchActionExecutedEvent,
					WorkbenchActionExecutedClassification
				>('workbenchActionExecuted', { id: CHAT_SETUP_ACTION_ID, from: 'accounts' });
