...
export class ChatWidget extends Disposable implements IChatWidget {
	...
	render(parent: HTMLElement): void {
		...
		this._register(
			dom.addStandardDisposableListener(this.welcomeMessageContainer, dom.EventType.CLICK, () =>
				this.focusInput()
			)
		);
		if (renderInputOnTop) {
	...
	setModel(model: IChatModel, viewState: IChatViewState): void {
		...
		this.viewModelDisposables.add(
			Event.runAndSubscribe(Event.accumulate(this.viewModel.onDidChange, 0), events => {
				if (!this.viewModel) {
					return;
				}
				...
				if (
					!requestInput.attachedContext.some(
						variable =>
							...
							isEqual(IChatRequestVariableEntry.toUri(variable), uri)
					)
				) {
	...
	private async _acceptInput(
		...
		if (this.viewModel) {
			...
			const requestInputs = {
				...
				attachedContext: await this.inputPart.getAttachedAndImplicitContext(
	...
	private async autoAttachInstructions(
		...
		const variableUris = attachedContext.map(IChatRequestVariableEntry.toUri).filter(isDefined);
