...
class TestPromptFileReference extends Disposable {
	 ...
	 * Run the test.
	 */
	public async run(options: Partial<IPromptParserOptions> = {}): Promise<FilePromptParser> {
		...
		const rootReference = this._register(
			this.instantiationService.createInstance(FilePromptParser, this.rootFileUri, options)
		).start();
		...
		for (let i = 0; i < this.expectedReferences.length; i++) {
			...
			assert(
				resolvedReference && resolvedReference.range.equalsRange(expectedReference.range),
				[
...
suite('PromptFileReference', function () {
			...
			instantiationService.createInstance(
				...
				[
					...
					new ExpectedReference(
						...
						new MarkdownLink(2, 1, '[]', '(./some-other-folder/non-existing-folder)'),
						new OpenFailed(
					...
					new ExpectedReference(
						...
						new MarkdownLink(1, 1, '[caption]', `(/${rootFolderName}/folder1/some-other-folder)`),
						new FolderReference(
							URI.joinPath(rootUri, './folder1/some-other-folder'),
			...
			const test = testDisposables.add(
				instantiationService.createInstance(
					...
					[
						...
						new ExpectedReference(
							URI.joinPath(rootUri, './folder1/some-other-folder'),
							new MarkdownLink(1, 1, '[]', `(/${rootFolderName}/folder1/some-other-folder)`),
							new FolderReference(
								URI.joinPath(rootUri, './folder1/some-other-folder'),
								'This folder is not a prompt file!'
							)
	...
	suite('• metadata', () => {
		test('• tools', async function () {
			...
			const test = testDisposables.add(
				instantiationService.createInstance(
					...
					[
						...
						new ExpectedReference(
							URI.joinPath(rootUri, './folder1'),
							new MarkdownLink(5, 1, '[]', '(./some-other-folder/non-existing-folder)'),
							new OpenFailed(
								URI.joinPath(rootUri, './folder1/some-other-folder/non-existing-folder'),
								'Reference to non-existing file cannot be opened.'
							)
						...
						new ExpectedReference(
							URI.joinPath(rootUri, './folder1/some-other-folder'),
							new MarkdownLink(4, 1, '[]', `(/${rootFolderName}/folder1/some-other-folder)`),
							new FolderReference(
								URI.joinPath(rootUri, './folder1/some-other-folder'),
								'This folder is not a prompt file!'
							)
			...
			assert.deepStrictEqual(tools, ['my-tool1'], 'Must have correct tools metadata.');
			...
			assertDefined(allToolsMetadata, 'All tools metadata must to be defined.');
			assert.deepStrictEqual(
				allToolsMetadata,
		...
		suite('• applyTo', () => {
			test('• prompt language', async function () {
				...
				assert.deepStrictEqual(tools, ['my-tool12'], "Must have correct 'tools' metadata.");

				assert.strictEqual(mode, ChatMode.Agent, "Must have correct 'mode' metadata.");
				...
				assert.strictEqual(applyTo, undefined, "Must have no 'applyTo' metadata.");
			});
			...
			test('• instructions language', async function () {
				...
				assert.strictEqual(applyTo, '**/*', "Must have no 'applyTo' metadata.");
			});
		});
		...
		suite('• tools and mode compatibility', () => {
			test('• tools are ignored if root prompt in the ask mode', async function () {
				...
				assert.deepStrictEqual(tools, undefined, "Must have correct 'tools' metadata.");

				assert.deepStrictEqual(mode, ChatMode.Ask, "Must have correct 'mode' metadata.");
				...
				assert.deepStrictEqual(allToolsMetadata, null, 'Must have correct all tools metadata.');
			});
			...
			test('• tools are ignored if root prompt in the edit mode', async function () {
				...
				assert.deepStrictEqual(mode, ChatMode.Edit, 'Must have correct tools metadata.');
			...
			test('• tools are not ignored if root prompt in the agent mode', async function () {
				...
				assert.deepStrictEqual(mode, ChatMode.Agent, "Must have correct 'mode' metadata.");
