...
CommandsRegistry.registerCommand({
	...
	handler: async (accessor, id: string) => {
		...
		if (extensionToUninstall.isBuiltin) {
			throw new Error(
				localize('builtin', "Extension '{0}' is a Built-in extension and cannot be uninstalled", id)
			);
		}
...
class ExtensionToolsContribution extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		this._register(
			toolsService.registerToolImplementation(SearchExtensionsToolData.id, searchExtensionsTool)
		);
		...
		this._register(
			toolsService.registerToolImplementation(InstallExtensionsToolData.id, installExtensionsTool)
		);
	}
}
