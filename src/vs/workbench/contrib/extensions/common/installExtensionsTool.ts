...
export const InstallExtensionsToolData: IToolData = {
	...
	modelDescription: localize(
		...
		"This is a tool for installing extensions in Visual Studio Code. You should provide the list of extension ids to install, and the confirmation message that is shown to the user. The identifier of an extension is '\${ publisher }.\${ name }' for example: 'vscode.csharp'."
	),
	...
	async prepareToolInvocation(
		...
		return {
			confirmationMessages: {
				title:
					parameters.confirmation?.title ??
					localize('installExtensionsTool.confirmationTitle', 'Install Extensions'),
				message:
					parameters.confirmation?.message ??
					localize(
						'installExtensionsTool.confirmationMessage',
						'These extensions are recommeded for you by Copilot.'
					),
			},
			toolSpecificData: {
				kind: 'extensions',
