	...
	constructor(
		...
		this._register(
			chatContextPickService.registerChatContextItem(
				new KernelVariableContextPicker(this.editorService, this.notebookKernelService)
			)
		);
...
export class SelectAndInsertKernelVariableAction extends Action2 {
	...
	override async run(accessor: ServicesAccessor, ...args: any[]): Promise<void> {
		...
		const pickedVariable = await quickInputService.pick(quickPickItems, {
			placeHolder: localize('selectKernelVariablePlaceholder', 'Select a kernel variable'),
		});
		if (!pickedVariable) {
			return;
		}
