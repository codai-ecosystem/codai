	...
	public start({
		...
		return this._connectionSequencer.queue(async () => {
			...
			if (
				...
				await Promise.all(
					this._mcpRegistry.delegates.get().map(r => r.waitForInitialProviderPromises())
				);
...
class McpPrompt implements IMcpPrompt {
	...
	constructor(
		...
		this.id = (this._server.definition.label + '.' + _definition.name).replace(
			/[^a-z0-9_.-]/gi,
			'_'
		);
	...
	_callWithProgress(
		...
		return McpServer.callOn(
			...
			h => {
				...
				return h
					.callTool({ name, arguments: params, _meta: { progressToken } }, token)
