...
export class McpResourcePickHelper {
	...
	public getPicks(
		onChange: (value: Map<IMcpServer, (IMcpResourceTemplate | IMcpResource)[]>) => void,
		...
		return Promise.all(
			this._mcpService.servers.get().map(async server => {
			...
			.getPicks(servers => {
				...
				for (const [server, resources] of servers) {
					...
					for (const resource of resources) {
						...
						if (chatWidget) {
							pickItem.buttons = [
								{ iconClass: ThemeIcon.asClassName(Codicon.attach), tooltip: attachButton },
							];
						}
