...
export class TerminalInstance extends Disposable implements ITerminalInstance {
	...
	get description(): string | undefined {
		...
		const type =
			this.shellLaunchConfig.attachPersistentProcess?.type || this.shellLaunchConfig.type;
		switch (type) {
			case 'Task':
		...
		this._register(
			this._configurationService.onDidChangeConfiguration(async e => {
				...
				if (e.affectsConfiguration(TerminalSettingId.KillGracefully)) {
					this.shellLaunchConfig.killGracefully = this._configurationService.getValue(
				...
				switch (typeof waitOnExit) {
					...
					case 'function':
						if (this.exitCode !== undefined) {
							xterm.raw.write(
								formatMessageForTerminal(waitOnExit(this.exitCode), { excludeLeadingNewLine: true })
							);
						}
						break;
				}
