...
suite('ExtensionsActivator', () => {
	...
	test('calls activate only once with parallel activations', async () => {
		...
		const activator = createActivator(host, [desc(idA, [], ['evt1', 'evt2'])], [], disposables);
	...
	test('Supports having resolved extensions', async () => {
		...
		const activator = createActivator(host, [desc(idA, [idB])], [bExt], disposables);
	...
	test('Error: activateById with missing extension', async () => {
		const disposables = new DisposableStore();
		const host = new SimpleExtensionsActivatorHost();
		const activator = createActivator(host, [desc(idA), desc(idB)], [], disposables);
	...
	test('Error: dependency missing', async () => {
		const disposables = new DisposableStore();
		const host = new SimpleExtensionsActivatorHost();
		const activator = createActivator(host, [desc(idA, [idB])], [], disposables);
	...
	test('Error: dependency activation failed', async () => {
		...
		const activator = createActivator(host, [desc(idA, [idB]), desc(idB)], [], disposables);
	...
	function createActivator(
		...
		const registry = disposables.add(
			new ExtensionDescriptionRegistry(basicActivationEventsReader, extensionDescriptions)
		);
		...
		return disposables.add(
			new ExtensionsActivator(registry, globalRegistry, host, new NullLogService())
		);
	}
