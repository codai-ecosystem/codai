...
export class ThemeMainService extends Disposable implements IThemeMainService {
	...
	private updateWindowSplashOverride(
		...
		if (workspace) {
			...
			changed =
				this.doUpdateWindowSplashOverride(workspace, splash, splashOverride, 'auxiliaryBar') ||
				changed;
		}
	...
	private doUpdateWindowSplashOverride(
		...
		if (currentWidth > 0) {
			if (overrideWidth !== currentWidth) {
				splashOverride.layoutInfo[part === 'sideBar' ? 'sideBarWidth' : 'auxiliaryBarWidth'] =
					currentWidth;
				changed = true;
			}
	...
	private doGetWindowSplash(
		...
		const auxiliarySideBarDefaultVisibility = this.configurationService.getValue(
			AUXILIARYBAR_DEFAULT_VISIBILITY
		);
