...
function toExtension(
	...
	return {
		...
		hasPreReleaseVersion:
			hasPreReleaseForExtension(id, productService) ?? isPreReleaseVersion(latestVersion),
...
export abstract class AbstractExtensionGalleryService implements IExtensionGalleryService {
	...
	private async getResourceApi(
		...
		const value = updateCheck
			...
			: ((await this.assignmentService?.getTreatment<'unpkg' | 'marketplace' | 'none'>(
					'extensions.gallery.useLatestApi'
				)) ?? 'unpkg');
	...
	private async isEngineValid(
		...
		if (!engine) {
			...
			try {
				...
				>('galleryService:engineFallback', { extension: extensionId, extensionVersion: version });
