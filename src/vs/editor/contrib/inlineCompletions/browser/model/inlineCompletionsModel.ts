...
export class InlineCompletionsModel extends Disposable {
	...
	constructor(
		...
		this._fetchSpecificProviderSignal = observableSignal<InlineCompletionsProvider | undefined>(
			this
		);
		...
		this._fetchInlineCompletionsPromise = derivedHandleChanges(
			...
			(reader, changeSummary) => {
				...
				const providers = changeSummary.provider
					...
					: this._languageFeaturesService.inlineCompletionsProvider.all(this.textModel);
		...
		>(
			...
			reader => {
				...
				if (inlineEditResult) {
					...
					const e = edits
						? TextEdit.fromStringEdit(edits, new TextModelText(this.textModel)).replacements
						: [edit];
	...
	public jump(): void {
		...
		transaction(tx => {
			...
			const isSingleLineChange =
				...
				(s.inlineCompletion.displayLocation || !s.inlineCompletion.insertText.includes('\n'));
