		...
		await test.run(
			...
			[
				...
				new FrontMatterRecord([
					new FrontMatterRecordName([new Word(new Range(1, 1, 1, 1 + 4), 'just')]),
				...
				// third record
				new FrontMatterRecord([
					new FrontMatterRecordName([new Word(new Range(3, 1, 3, 1 + 12), 'anotherField')]),
					new FrontMatterRecordDelimiter([
			...
			test('• unquoted string', async () => {
				...
				await test.run(
					...
					[
						...
						// second record
						new FrontMatterRecord([
							new FrontMatterRecordName([new Word(new Range(2, 1, 2, 1 + 12), 'anotherField')]),
							new FrontMatterRecordDelimiter([
								new Colon(new Range(2, 17, 2, 18)),
								new Space(new Range(2, 18, 2, 19)),
							]),
							new FrontMatterSequence([new Word(new Range(2, 20, 2, 20 + 3), 'fal')]),
						]),
			...
			test('• boolean', async () => {
				...
				await test.run(
					...
					[
						// first record
						new FrontMatterRecord([
							new FrontMatterRecordName([new Word(new Range(1, 1, 1, 1 + 12), 'anotherField')]),
							new FrontMatterRecordDelimiter([
				...
				test('• empty', async () => {
					...
					await test.run(
						...
						[
							// first record
							new FrontMatterRecord([
								new FrontMatterRecordName([new Word(new Range(1, 1, 1, 1 + 5), 'tools')]),
								new FrontMatterRecordDelimiter([
