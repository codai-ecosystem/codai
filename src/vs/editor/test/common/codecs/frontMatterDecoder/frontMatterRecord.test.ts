...
suite('FrontMatterBoolean', () => {
	...
	suite('• trimValueEnd()', () => {
		test("• trims space tokens at the end of record's value", () => {
			const recordName = new FrontMatterRecordName([new Word(new Range(4, 10, 4, 10 + 3), 'key')]);
			...
			const record = new FrontMatterRecord([recordName, recordDelimiter, recordValue]);
