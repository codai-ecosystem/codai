...
export class TreeSitterTokenizationImpl extends Disposable {
	...
	private readonly _onDidChangeTokens: Emitter<{ changes: IModelTokensChangedEvent }> =
		this._register(new Emitter());
	public readonly onDidChangeTokens: Event<{ changes: IModelTokensChangedEvent }> =
	...
	private _handleTreeUpdate(ranges: RangeChange[], versionId: number) {
		...
		return this._updateTreeForRanges(rangeChanges, versionId, captures).then(() => {
			if (
				...
				this._tree.treeLastParsedVersion.get() === this._textModel.getVersionId()
			) {
