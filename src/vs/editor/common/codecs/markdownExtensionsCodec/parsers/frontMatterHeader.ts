...
> {
	...
	): TAcceptTokenResult<PartialFrontMatterStartMarker | PartialFrontMatterHeader> {
	...
	public static mayStartHeader(token: TSimpleDecoderToken): token is Dash {
		return (
			token instanceof Dash && token.range.startLineNumber === 1 && token.range.startColumn === 1
		);
	}
}
...
> {
	...
	public override get tokens(): readonly TSimpleDecoderToken[] {
		const endMarkerTokens = this.maybeEndMarker !== undefined ? this.maybeEndMarker.tokens : [];

		return [...this.startMarker.tokens, ...this.currentTokens, ...endMarkerTokens];
	}
